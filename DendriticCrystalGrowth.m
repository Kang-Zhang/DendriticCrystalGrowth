...
for i=1:nx
    for j=1:ny
        phi1(i,j)=0;
        if ((i-nx/2)*(i-nx/2)+(j-ny/2)*(j-ny/2)<20);
            phi1(i,j) = 1;
        end
    end
end
...
% grad_epsilon2_x = (epsilon(ip,j)^2 - epsilon(im,j)^2)/dx;%%%%%%%%
% grad_epsilon2_y = (epsilon(i,jp)^2 - epsilon(i,jm)^2)/dy;%%%%%%%%
term1=(epsilon(i,jp)*epsilon_derivative(i,jp)*grad_phi_x(i,jp) - epsilon(i,jm)*epsilon_derivative(i,jm)*grad_phi_x(i,jm))/dy;
term2= -(epsilon(ip,j)*epsilon_derivative(ip,j)* grad_phi_y(ip,j) - epsilon(im,j)*epsilon_derivative(im,j)*grad_phi_y(im,j))/dx;
